...
class GantryLayoutChrome_Popup extends GantryLayout {
    ...
    function render($params = array()){
        ...
        $params[0]['widget_open'] = sprintf('<div id="%1$s" class="widget %2$s rt-block"><div class="module-content">', $id, $classname);
        $params[0]['widget_close'] = '</div></div>';
        
        $params[0]['pre_render'] = '<div class="module-inner">';
        $params[0]['post_render'] = '</div>';

        $params[0]['title_open'] =  '<h2 class="title">';
        $params[0]['title_close'] = '</h2>';
